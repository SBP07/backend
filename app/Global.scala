...
object Global extends GlobalSettings {
  ...
  override def onStart(app: Application) {
    ...
    play.api.db.slick.DB.withSession{ implicit session =>
      if(Children.count == 0)
        Children.insert(Child(Some(4), "Thomas", "Toye", Option("0455 55 55 55"), Option("056/55 55 55"), Option("Tomberg 21A"), Option("Beveren-Leie"), Option(new Date()), true, Option(new Date())))
    }
